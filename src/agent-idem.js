...
export function appendAgentIdempotent(db, { key, appendArgs, now = Date.now() }) {
  return db.transaction(() => {
    ...
    const prior = db.prepare(
      'SELECT seq FROM agent_idem WHERE key=? AND expires_at>?'
    ).get(key, now)
    ...
    db.prepare(
      'INSERT INTO agent_idem(key, seq, expires_at) VALUES(?,?,?)'
    ).run(key, result.seq, now + AGENT_IDEM_TTL_MS)
    return result
  })()
}
