...
CREATE TABLE IF NOT EXISTS conversations(
  ...
  session_outcome TEXT
    CHECK(session_outcome IS NULL OR session_outcome IN ('completed','interrupted','failed')),
...
export function openDb(path) {
  ...
  if (!convoCols.some((c) => c.name === 'session_outcome')) {
    db.exec("ALTER TABLE conversations ADD COLUMN session_outcome TEXT CHECK(session_outcome IS NULL OR session_outcome IN ('completed','interrupted','failed'))")
  }
